...
def test_train_split(dataset,folds):
    ...
    for f in range(folds):
        ...
        test_t = dataset[f] # test data (test-cross validation split)
        split.remove(test_t)
        test_split=[]
        train_split=[]
        for t in range(f_size):
            test_split.append(test_t[t][0][:])
        for i in range(folds-1): # handling the training data part (train- cross validation-split)
            for j in range(f_size):
                train_t=split[i][j][0]
                train_split.append(train_t)# appending the training data in train_split list
        for x in range(len(test_split)):
            if(len(test_split[x][:])==11):
                del test_split[x][1]
            for y in range(1,9,1):
                #print test_split[x][y]
                #print y
                test_split[x][y]=float(test_split[x][y])
        for x in range(len(train_split)):
            if(len(train_split[x][:])==11):
                del train_split[x][1]
            for y in range(1,9,1):
...
def main():
   ...
   #k=3
   #predictions=[]
   load_resample_data('yeast.data',training_set,test_set)
   mean_accuracy()
